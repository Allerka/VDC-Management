...
function getVehicle(data) {
  // const data = JSON.parse(getStorage("viewRecords"));
  var vehicleName = null;
  if (data) {
    vehicleName = String(data.test_name);
  }
  return (
    vehicleName
  )
...
export function SubHeader() {
  ...
  useLayoutEffect(() => {
    ...
    subheader.setTitle(getStorage("pageTitle"));
  ...
  store.subscribe(() => {
    // setCurrentStep(parseInt(getStorage("currentStep")));
    // setVehicleName(getStorage("viewRecords"));
    subheader.setTitle(getStorage("pageTitle"));
      })
